import { AppBar, Box, IconButton, TextField, Toolbar, useTheme } from '@mui/material'
...
const MessageBar = ({ rounded, emojiOpen, setEmojiOpen }: { rounded?: boolean; emojiOpen: boolean; setEmojiOpen: Dispatch<SetStateAction<boolean>> }) => {
  ...
  const sendMessage = () => {
    if (inputValue !== '') {
      const msg = JSON.stringify({ type: 'chat', content: inputValue, recipients: chat?.members || ['General'], chatId: chat?.id, msgId: uuidv4() })
      ws?.send(msg)
      setInputValue('')
      setEmojiOpen(false)
    }
  ...
  return (
    <>
      ...
      <Box sx={{ display: 'flex', position: 'relative', padding: 0 }}>
        <AppBar color='inherit' position='sticky' style={{ bottom: 0 }} sx={{ borderRadius: rounded ? '0 0 12px 12px' : 0 }}>
          <Toolbar>
            <IconButton
              color='inherit'
              onClick={() => {
