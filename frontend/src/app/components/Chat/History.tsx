...
import { formatedDayDate } from '@/lib/utils'
                  ...
                  <PopupState variant='popover' popupId='reactions-popover'>
                    {(popupState) => (<>
                      <MessageWrapper minWidth={(reactions?.length || 1) * 30} invert={isMe} {...bindHover(popupState)}>
                        {time && <Message group={group} invert={isMe} author={author} content={content} time={time} onClick={() => group && openPrivateChat(author)} />}
                        {groupedReactions && Object.values(groupedReactions).map((r: IReactionWithCount, i: number) => (
